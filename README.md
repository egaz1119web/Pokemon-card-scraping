...
## 検索に出てこないカード（cardId の穴）

**カード検索は、詳細ページのあるカードをすべて返すわけではない。**

例: 049573「ポケパッド」（スタートデッキ100 バトルコレクション）は
詳細ページも画像もあるのに、`resultAPI.php` ではどのレギュレーションでも
ヒットしない。キーワード「ポケパッド」で引くと 9 件返るが、そこに 049573 は
入っていない。

一覧に出ないだけなら害は無さそうに見えるが、**公式のデッキコードは
こういうカードを平気で指す**。アプリのデッキコード取り込み
（`connect/Connect.kt`）は `deck_gds` などの値から cardId をそのまま取るので、
検索由来の `cards.json` に無い cardId がユーザーのデッキに入る。
その状態で共有すると、デッキ共有ページはカード名も絵も引けず、
「カード 49573」という灰色の枠が出る。実際にそう報告された。

そこで、持っている cardId の並びに空いた番号を順に叩いて確かめる。

```bash
npm run fill-gaps   # 穴を叩いて、見つかったカードを data/cards.json に足す
npm run build       # そのあと必ず回す（配信ファイルと version を作り直す）
```

- 存在しない番号は 302 で検索ページへ流されるので、カード画像を含まない
  応答は「無い」と見なして `data/gap-misses.json` に控え、次回から叩かない。
- 43000 番より下は見ない（`GAP_FROM`）。1 番からの穴はほとんどが
  そもそもカードの無い番号で、3 万リクエスト投げてもほぼ空振りになる。
  データが詰まっているのは 43000 番以降で、そこから上の穴は初回で 270 件だった。
- 拾ったカードは `standard` も `extra` も false になる（どちらの検索にも
  出ないため）。アプリのカード検索はこの印で絞っているので**検索結果は増えない**。
  デッキの表示とデッキコードの取り込みは全件から引くので、そこだけが直る。
- `fill-gaps` は `data/state.json` の `pendingBump` を立てる。`build.ts` は
  「読み込んだ `data/cards.json`」と「組み直した結果」を比べて変化を見るため、
  先に master を書いてしまうと差が出ず、version が据え置かれてアプリに届かない。

初回（2026-09-04, version 56）に拾った 270 件の内訳:

| 範囲 | 件数 | 収録 |
|---|---|---|
| 049467〜049584 | 34 | スタートデッキ100 バトルコレクション |
| 049718〜049953 | 236 | ハイクラスパック「MEGAドリームex」 |
...
npm run fill-gaps  # 検索に出てこないカードを cardId の穴から拾う（→ そのあと build）
...
| `MAX_FETCH` | 1000（fill-gaps は 500） | 1 回の実行で取得する上限 |
...
| `GAP_FROM` | 43000 | fill-gaps が穴を探し始める cardId |
